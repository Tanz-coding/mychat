    ...
    <div class="session-panel-footer" @mousedown.stop="">
      <div class="session-tool-warp">
        ...
        <label class="session-tool-item">
          ...
          <input type="file" accept="image/png, image/jpg, image/jpeg" @change="fileChange($event, 'image')">
        </label>
        <label class="session-tool-item">
          <span class="iconfont icon-plus-o"></span>
          <input type="file" accept=".txt,.pdf" @change="fileChange($event, 'file')">
        </label>
  ...
  export default {
    ...
    data(){
      return {
        ...
        isShow:false,
        text:"",
        uploadProgress: 0, // Add upload progress data property
        isUploading: false // Add a flag to indicate if a file is being uploaded
      }
    },
    methods:{
      toggleShow() {
      ...
      fileChange(e, type){
        ...
        if (type === 'image') {
          ...
          const reg = /\.(?:txt|pdf)$/i;
          if(!reg.test(file.name)){
            Message.warning("请选择正确格式的文本或PDF文件!");
            return
          }
          let maxSize=5*1024*1024; // 5MB limit for files
          if(file.size>maxSize){
            Message.warning("文件大小不能超过5M!","warning");
            return
          }
          ...
          this.$axios.post('/upload/file', formData, {
            ...
            onUploadProgress: progressEvent => {
              this.uploadProgress = Math.round((progressEvent.loaded * 100) / progressEvent.total); // Update progress
            }
          }).then(res => {
            if (res.data && res.data.filePath) {
              this.sendMessage(res.data.filePath, 'file');
