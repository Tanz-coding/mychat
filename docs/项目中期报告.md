...
本文档从分组情况、数据源、选用技术及作品初始设计四个方面说明。报告基于代码实际实现撰写，引用的路径均以 `mychat/` 为根。
...
- 前端单页（Vue）+ 后端 Node.js & WebSocket（Express + Socket.IO）
- 数据分层：
  - 聊天域：NeDB 文件存储，快速易用
...
```
{
	...
	"upload": {
		"maxFileSizeMB": 10,
		"allowedExtensions": [".jpeg", ".jpg", ".png", ".gif", ".txt", ".pdf"],
		"allowedMimeTypes": [
			"image/jpeg",
			"image/jpg",
			"image/png",
